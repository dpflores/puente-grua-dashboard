...
var nombresMeses = [
    ...
    function generateTrace(hours, month, fInit, fEnd, state, sw){
        ...
        if (state == "init"){ var colorT = 'rgba(216,217,219,0.8)'; var colorB='rgba(98,106,106,1)'}
        if (state == "positivo"){ var colorT = 'rgba(14,18,113,0.8)'; var colorB = 'rgba(14,18,113,1)';}
        if (state == "negativo"){ var colorT = 'rgba(0,148,206,0.8)'; var colorB = 'rgba(0,148,206,1)';}
        ...
        var trace = {
          ...
          hovertemplate: '<b>'+state +'</b>'+'<br>'+'<br>'+ '<b>Horas</b>: %{x:.2f}' +
          '<br>%{text}' +'<extra></extra>'
        }
...
var traceInitUE = generateTrace(0.0001, mes_init, '000/00/00 00:00', '0000/00/00 00:00', 'positivo', true)
var traceInitNU = generateTrace(0.0001, mes_init, '000/00/00 00:00', '0000/00/00 00:00', 'negativo', true)
var traceInitUC = generateTrace(0.0001, mes_init, '000/00/00 00:00', '0000/00/00 00:00', 'init', true)
...
export default function FrenquencyChart() {
  ...
  return (
    <div className="bg-white p-4 rounded-sm border border-gray-200 flex flex-col flex-1">
        <strong className='text-gray-700 font-medium'> Frecuencia de uso mensual</strong>
...
function PlotlyChart({data}){
    return(
        <Plot
            data={data}
            layout={ {//title: 'FRECUENCIA DE USO MENSUAL',
